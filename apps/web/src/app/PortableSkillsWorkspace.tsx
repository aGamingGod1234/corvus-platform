...
export function PortableSkillsWorkspace({ api }: { api: PortableSkillsApi }) {
  ...
  return <section aria-labelledby="portable-skills-title" className="portable-skills-workspace">
    ...
    {preview ? <div className="skill-review-backdrop" role="presentation"><section aria-labelledby="skill-review-title" aria-modal="true" className="skill-review" role="dialog"><header><div><p className="eyebrow">Review import</p><h2 id="skill-review-title">{preview.name}</h2><p>{preview.description}</p></div><button aria-label="Close skill review" className="icon-button" onClick={() => setPreview(null)} type="button">×</button></header><div className="skill-review__facts"><span data-status={preview.compatibility}>{preview.compatibility.replaceAll("_", " ")}</span><span>{preview.candidate.source}</span><span>{preview.files.length} files</span><span>{preview.duplicate === "none" ? "New skill" : `${preview.duplicate} duplicate`}</span></div><code title={preview.digest}>{preview.digest}</code>{preview.findings.length ? <div className="skill-findings"><h3>Review findings</h3>{preview.findings.map((finding, index) => <article data-severity={finding.severity} key={`${finding.code}-${index}`}><strong>{finding.message}</strong><small>{finding.location} · {finding.code}</small></article>)}</div> : <p className="skill-review__clean">No compatibility or security findings. Import still copies the package without executing it.</p>}<footer><button className="button" onClick={() => setPreview(null)} type="button">Cancel</button><button className="button button--primary" disabled={busy || preview.compatibility === "blocked" || preview.duplicate === "exact"} onClick={() => void importSkill()} type="button">{preview.duplicate === "exact" ? "Already imported" : busy ? "Importing…" : "Import as draft"}</button></footer></section></div> : null}
  </section>;
}
