...
- `authority_state_root` is a canonical Merkle root under that manifest over deployment-instance activation/revocation, workspace authority and trust-anchor state, epoch-credential status, lease-certificate revocation, locally mirrored registry-verifier/trust-head state, workspace-signing-key lifecycle, policy, membership/access/agent/delegation and credential grants/revocations, authorization decisions, budget periods/scope/account/reservation/settlement state, kill switches, approvals, workflow/effect state, and the new audit-chain head. Registry workspace anchors/trust-state metadata and sealed local anchors are `external_proof` families bound by exact digests/versions in the commit receipt rather than recursively included. Every authoritative mutation or revocation advances a covered leaf or its named external proof; tests independently roll back every family and startup rejects a schema/manifest mismatch.
...
- `BudgetReservationSet(id, workspace_id, effect_intent_id, request_context_id, authorization_snapshot_id, leaf_account_id, state[held|claimed|settled|released|expired], expires_at, version)` has unique `effect_intent_id`; one external-effect intent exclusively owns one reservation set and a set cannot move to another intent or permit.
- `BudgetReservation(id, workspace_id, reservation_set_id, account_id, amount, state[held|claimed|settled|released|expired], expires_at, version)` has unique `(reservation_set_id, account_id)`. Set creation locks and reserves the leaf plus every current canonical account ancestor atomically; partial sets and reused rows fail.
- `BudgetUsageSettlement(id, workspace_id, reservation_id, effect_attempt_id?, reserved_amount, actual_amount, released_amount, state[settled|reversed], settled_at)` has unique `reservation_id`; a reservation may be consumed/settled/released exactly once, and account counters plus settlement append update atomically with conserved `actual_amount + released_amount = reserved_amount`.
...
- `EffectPermit(id, effect_intent_id, workspace_id, reservation_set_id, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, lease_fence, authorization_snapshot_id, effect_payload_version_id, ciphertext_sha256, canonical_plaintext_hmac, kill_switch_versions, state[available|claimed|cancelled|consumed], claimed_by?, claimed_at?, consumed_at?)`; `effect_intent_id` and `reservation_set_id` are each globally unique, so one intent has at most one permit and no reservation set can authorize two permits.
- `EffectPermitBudgetReservation(permit_id, workspace_id, reservation_set_id, reservation_id, account_id, account_version, reservation_version)` has unique `(permit_id, account_id)`, unique `reservation_id`, and composite foreign keys requiring the permit, set, reservation, and account to share one workspace/set. Permit creation proves this normalized set equals the complete current canonical account closure before it becomes `available`.
...
- Only the centralized effect gateway may dispatch external effects. Permit claim locks records in canonical order: workspace authority/commit receipt, sorted kill-switch rows, sorted budget-period/scope/account/closure/reservation-set/reservation rows, effect intent, permit plus normalized permit-reservation rows, then outbox. In one serializable transaction it verifies the current external/sealed authority generation/root, request and immutable authorization snapshot, requester/access/agent/delegation grants, placement, provider/credential grant/version, non-overlapping budget period, same-unit canonical scope/period containment, exclusive one-intent/one-permit reservation-set ownership, complete ancestor-reservation closure, kill switches, lease fence, immutable payload-version fields, and semantic key. It atomically moves the set and every row `held -> claimed` exactly once before claiming the permit. Immediately before provider I/O, it content-addresses the ciphertext artifact, verifies `ciphertext_sha256` and encryption/commitment key versions, decrypts and schema-validates/recanonicalizes the exact plaintext, verifies `canonical_plaintext_hmac`, derives `dispatch_request_digest`, and requires identical payload IDs/digests on intent, permit, outbox, attempt, audit, and lineage. Kill-switch activation atomically prevents new permit claims, cancels available permits, and marks already-claimed attempts for provider cancellation or compensation; no design claims that a remote provider can undo an effect it already accepted. Success/failure/unknown outcome, one-time conserved budget settlement/release, lineage, and audit append are transactional.
...
- Effective availability is the minimum remaining allowance across the selected leaf and its complete canonical account closure. One effect intent exclusively owns one `BudgetReservationSet`; creation locks the sorted closure and atomically reserves every account or fails all.
- Each effect permit exclusively owns that set and uses normalized `EffectPermitBudgetReservation` rows with global reservation uniqueness. The gateway proves the bound rows equal the complete current closure/versions and atomically claims them once; two permits cannot share any reservation or capacity.
- Settlement is unique per reservation, conserves reserved amount into actual plus released amount, and releases only unused capacity; cancellation never erases consumed usage. Database constraints, canonical locks and serializable race tests cover cross-unit/unrelated-scope links, different-period containment, rollover, hierarchy mutation, shared-set permit races, reserve/claim/settle/release/kill and concurrent effects.
...
2. Build deployment fixtures for first install, version upgrade, database/artifact backup and quarantined restore/new-epoch takeover, key rotation, worker loss, region/service degradation, migration failure, and rollout rollback.
...
4. Drive self-hosted first-admin bootstrap/recovery and configured OIDC, plus vendor-cloud OIDC Authorization Code + PKCE, through the retained CLI **HTTP** client and actual generated React client against deterministic identity-provider/deployment fixtures. No direct API/service fixture can substitute for callback, session-cookie/CSRF, token/session rotation, revocation, reconnect, upgrade, backup/restore quarantine, and new-epoch recovery tests.
5. Re-run project/work/provider/OAuth/evidence/event-replay flows through those real clients in both topologies; run signed channel ingress/step-up tests against the same authenticated deployments. Keep each client disabled in the matrix until its own flow passes.
6. Require signed release provenance, SBOM binding, health gates, staged rollout, and a documented disaster-recovery objective before either topology is called supported.

Checkpoint: self-hosted and vendor-cloud test environments each complete authenticated real-CLI and real-web install/bootstrap-or-OIDC -> callback/session/CSRF -> execute -> rotate/revoke/reconnect -> upgrade -> backup -> quarantined restore/new-epoch recovery -> rollback flows; adversarial tenant-isolation, failed-rollout, stale-session/token, callback, and restore tests pass with verifiable receipts. CLI/web/channel support is enabled only after these no-substitute client gates; desktop remains unsupported.
...
5. Through the actual Tauri client, rerun local-daemon pairing and the Milestone 10 self-hosted bootstrap/OIDC plus vendor-cloud PKCE flows, including callback, OS-keystore session, CSRF-equivalent origin controls, rotation/revocation, restart/reconnect, upgrade, restore quarantine and new-epoch recovery; raw HTTP fixtures cannot enable desktop support.

Checkpoint: desktop runs the proven local, self-hosted and cloud team/provider flows through generated clients without duplicate authority code or generic shell/filesystem access; authenticated lifecycle tests pass for each topology; every artifact verifies provenance/SBOM, installs, rejects rollback/freeze attacks, updates through threshold-signed metadata, survives key rotation/revocation fixtures, and rolls back through a tested path. Only then is desktop enabled for the corresponding matrix row.
...
- Self-hosted and vendor-cloud CLI/web/channel modes are not supported until actual retained-CLI HTTP and generated-React clients pass bootstrap/OIDC callback, session/CSRF, rotation/revocation, reconnect, upgrade, quarantined restore/new-epoch recovery and tenant-isolation gates in each deployment; desktop waits for Milestone 11 to rerun those flows through Tauri.
...
| Deployment operations | separate self-host/cloud install and upgrade, PostgreSQL/workers/artifacts/vault/KMS, tenant isolation, backup/restore/DR, observability, staged rollout rollback, real retained-CLI HTTP and generated-React bootstrap/OIDC callback/session/CSRF/rotation/revocation/reconnect/upgrade/restore flows |
| Desktop/distribution | Tauri capability audit, sidecar protocol, real-client local/self-host/cloud authenticated lifecycle reruns, TUF-style threshold metadata, expiry/rollback/freeze defense, key rotation/revocation, provenance/SBOM, OS/architecture CI |
...
- **Stale worker, payload substitution, budget write-skew or duplicate effects:** intent/permit/outbox/attempt/audit/lineage bind one immutable content-addressed payload version, ciphertext digest, encryption/commitment key versions and canonical-plaintext HMAC verified immediately before dispatch. Canonical locks, non-overlapping periods, typed real-owner scope ancestry, same-unit/containing windows, exclusive per-intent reservation sets, globally unique permit-reservation ownership, one-time conserved settlements, unique kill/permit/outbox/attempt constraints and exact authority generation close local races. Unknown provider outcomes never auto-retry without provider idempotency; compensation is remediation only.
...
- **Declared deployment/client modes without real authenticated proof:** keep self-hosted/vendor-cloud CLI/web/channel unsupported until PostgreSQL, workers, durable artifacts, vault/KMS, tenant isolation, backup/restore/DR, observability, migration and failed-rollout gates pass together with no-substitute retained-CLI HTTP and generated-React bootstrap/OIDC/session/CSRF/rotation/reconnect/restore flows. Keep desktop disabled until Tauri reruns those flows.
- **Resource pressure:** serialize Node/Rust builds and avoid local container builds while RAM is constrained.
- **Review availability:** Claude returned HTTP 401, Gemini required Antigravity migration, and Codex timed out without a verdict. Earlier paired Hermes rounds and their responses are recorded in `PLAN-REVIEW-LOG.md`. Exact commit `834341842a870fd723093bf511540cf6a35a6a69` received two `VERDICT: REVISE` results with six high findings: registry trust-state freshness, exhaustive state-root coverage, non-circular audit anchoring, semantic/unit-safe budget ancestry, exclusive reservation ownership, and real authenticated network-client gates. All are incorporated into this candidate. Both fresh scopes must return `VERDICT: PASS` on the same exact commit; no timeout, partial result or single pass is approval.
