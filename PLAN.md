...
|---|---|---|---|---|
| `embedded_local` | individual | CLI | local runner | Supported first only with SQLite, OS keyring, fixed workspace-wide OS lock, device-bound sealed monotonic generation/root and fail-closed sandbox; otherwise authority-bearing offline mutation is unsupported |
| `local_daemon` | individual | CLI after Milestone 4; web after Milestone 5; desktop after Milestone 11 | local runner | CLI topology requires the Milestone 4 daemon/auth/lifecycle gate; web remains unsupported until the real Milestone 5 client passes pairing, rotation, restart and reconnect Playwright tests |
...
- `AuthorityCommitReceipt(id, workspace_id, epoch, deployment_instance_id, generation, previous_state_root, state_root, mutation_digest, anchor_kind[registry|sealed_local], registry_sequence?, registry_signing_key_version_id?, registry_signature?, local_anchor_receipt_digest?, finalized_at)`; every authority-bearing transaction uses prepare -> external/sealed compare-and-swap -> database commit -> anchor finalize. A crash may replay only the exact prepared digest; missing/ambiguous state quarantines.
- `authority_state_root` is a canonical Merkle root over versioned policy, memberships/access/agent/delegation grants, credential grants/revocations, budget periods/accounts/reservations/settlements, kill switches, approvals, workflow/effect state, and audit chain head. Each authoritative transaction updates its affected leaves; a database snapshot with any older family cannot match the current external/sealed root.
...
- `AuditReceipt(id, workspace_id, workspace_sequence, schema_version, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, request_context_id, authorization_snapshot_id, authorization_snapshot_digest, action, resource, decision, reason_code, policy_digest, sanitized_input_digest, output_digest?, effect_payload_version_ids, effect_payload_commitment_digests, effect_attempt_ids, cost_json, evidence_ids, signing_key_version_id, previous_hash, receipt_hash, receipt_signature)`; `(workspace_id, workspace_sequence)` is unique and monotonic.
- `AuditCheckpoint(id, workspace_id, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, through_sequence, receipt_hash, schema_version, checkpoint_authorization_snapshot_id, checkpoint_authorization_snapshot_digest, covered_authorization_snapshot_set_digest, covered_effect_payload_set_digest, signing_key_version_id, signature, previous_checkpoint_digest?, anchored_at)`; receipt append, sequence allocation, chain-head update, and authority-generation commitment follow the prepared/finalized anchor protocol, while private signing keys live in OS keyring/cloud KMS and never in the ledger database.
...
- `ExternalEffectIntent(id, workspace_id, workflow_graph_id, work_item_id, work_attempt_id, request_context_id, authorization_snapshot_id, authorization_snapshot_digest, authorization_signing_key_version_id, requester_id, access_bundle_id, agent_grant_id, delegation_grant_ids, execution_placement_id, provider_connection_id, credential_grant_id, credential_version_id, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, lease_fence, semantic_idempotency_key, effect_kind, target_digest, effect_payload_version_id, payload_schema_version, payload_canonicalization_version, ciphertext_sha256, canonical_plaintext_hmac, required_capabilities_digest, kill_switch_snapshot_ids, state[pending|dispatching|succeeded|failed|outcome_unknown|cancelled|compensating|compensated], created_at)` with unique `(workspace_id, semantic_idempotency_key)`; the semantic key canonically binds workspace, effect kind, target, immutable payload-version ID/ciphertext/plaintext commitment, and authorization-snapshot digest, and reuse with any mismatch is rejected.
...
- `BudgetAccount(id, workspace_id, scope_kind[workspace|project|routine_channel|run|agent], scope_id, scope_rank, unit, budget_period_id, limit_amount, reserved_amount, settled_amount, status[active|frozen|closed], version)` with unique `(workspace_id, scope_kind, scope_id, unit, budget_period_id)` and checks `0 <= reserved_amount`, `0 <= settled_amount`, `reserved_amount + settled_amount <= limit_amount`, and `scope_rank` matches `workspace=0, project=1, routine_channel=2, run=3, agent=4`.
- `BudgetAccountParent(workspace_id, child_account_id, parent_account_id)` has unique `child_account_id`, composite same-workspace foreign keys, and a legal-order check `parent.scope_rank < child.scope_rank`; a child links to its nearest configured ancestor in workspace -> project -> routine/channel -> run -> agent order.
- `BudgetAccountClosure(workspace_id, ancestor_account_id, descendant_account_id, depth)` has unique `(ancestor_account_id, descendant_account_id)`, same-workspace foreign keys and check `(depth = 0) = (ancestor_account_id = descendant_account_id)`. Parent/closure insertion is one serializable transaction with cycle rejection.
- `BudgetReservation(id, workspace_id, account_id, request_context_id, authorization_snapshot_id, amount, state[held|settled|released|expired], expires_at, version)` with unique `(request_context_id, account_id)`; a reservation set is valid only when it covers the selected leaf account and every row in its transitive `BudgetAccountClosure`.
...
- `EffectPermit(id, effect_intent_id, workspace_id, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, lease_fence, authorization_snapshot_id, effect_payload_version_id, ciphertext_sha256, canonical_plaintext_hmac, kill_switch_versions, state[available|claimed|cancelled|consumed], claimed_by?, claimed_at?, consumed_at?)`; `effect_intent_id` is unique, so one intent has at most one permit.
- `EffectPermitBudgetReservation(permit_id, workspace_id, reservation_id, account_id, account_version, reservation_version)` has unique `(permit_id, account_id)`, composite same-workspace foreign keys, and a unique `(permit_id, reservation_id)`. Permit creation compares this normalized set to the complete transitive account closure before it can become `available`.
- `ExternalEffectAttempt(id, effect_intent_id, attempt_number, gateway_id, request_context_id, authorization_snapshot_id, authorization_signing_key_version_id, provider_connection_id, credential_grant_id, credential_version_id, authority_epoch, authority_generation, authority_state_root, authority_commit_receipt_id, lease_fence, authorization_receipt_id, effect_payload_version_id, ciphertext_sha256, canonical_plaintext_hmac, dispatch_request_digest, provider_idempotency_key?, state[started|succeeded|failed|outcome_unknown|cancelled|compensated], started_at, finished_at?, result_digest?, error_code?, compensation_attempt_id?)` with unique `(effect_intent_id, attempt_number)`.
- `EffectOutbox(id, effect_intent_id, effect_payload_version_id, ciphertext_sha256, canonical_plaintext_hmac, dispatch_after, state[pending|claimed|delivered|outcome_unknown|cancelled], fence, claimed_by?, delivered_at?)`; `effect_intent_id` is unique, so one intent has at most one outbox row.
- `LineageNode(id, workspace_id, kind[source|model_call|tool_call|test_evidence|approval|audit_receipt|authorization_snapshot|effect_payload|effect_attempt|artifact], immutable_record_id, canonical_digest)`
...
- Only the centralized effect gateway may dispatch external effects. Permit claim locks records in canonical order: workspace authority/commit receipt, sorted kill-switch rows, sorted budget-period/account/closure/reservation rows, effect intent, permit plus normalized permit-reservation rows, then outbox. In one serializable transaction it verifies the current external/sealed authority generation/root, request and immutable authorization snapshot, requester/access/agent/delegation grants, placement, provider/credential grant/version, non-overlapping budget period, complete ancestor-reservation closure, kill switches, lease fence, immutable payload-version fields, and semantic key before claiming the permit. Immediately before provider I/O, it content-addresses the ciphertext artifact, verifies `ciphertext_sha256` and encryption/commitment key versions, decrypts and schema-validates/recanonicalizes the exact plaintext, verifies `canonical_plaintext_hmac`, derives `dispatch_request_digest`, and requires identical payload IDs/digests on intent, permit, outbox, attempt, audit, and lineage. Kill-switch activation atomically prevents new permit claims, cancels available permits, and marks already-claimed attempts for provider cancellation or compensation; no design claims that a remote provider can undo an effect it already accepted.
...
- Budget layers are materialized `BudgetAccount` rows linked by same-workspace, single-parent, legal-rank `BudgetAccountParent` rows and transactionally maintained `BudgetAccountClosure` rows; cycles, cross-workspace parents, duplicate parents, illegal/reversed rank and parallel accounts for one scope/unit/period fail closed.
- Effective availability is the minimum remaining allowance across the selected leaf and its complete transitive ancestor closure. Reservation creation locks that sorted closure and atomically increments every account or fails all.
- Each effect permit uses normalized `EffectPermitBudgetReservation` rows. The gateway proves the bound reservation/account set equals the complete current closure and versions before dispatch; a list/digest alone is never authoritative.
- Settlement records actual use and releases only unused amounts; cancellation never erases consumed usage. Database checks, canonical locks and serializable race tests cover period rollover, hierarchy mutation, reserve/settle/release/kill and concurrent permit interactions.
...
1. Add `DeploymentProfile`, `DeploymentInstance`, `WorkspaceConfig`, non-rollback `WorkspaceAuthority` external/sealed generation anchors, exclusive leases/OS locks, registry-verifier history, epoch credentials, close/handoff/restore contracts, `ClientContext`, `ExecutionPlacement`, identity, discriminated scope, audience snapshots, requester/agent grants, immutable authorization-decision snapshots, versioned signing keys, `EffectiveCapabilities`, idempotency, and audit contracts with an explicit combination table.
2. Add `M1-001` and an idempotent migration-backed local project/deployment-instance/authority-generation/registry-key/audience/authorization-snapshot/signing-key/audit repository without mutating legacy V1 rows; add only the project/config portions of the per-domain importer from sealed quarantine. V1 policy/autonomy waits for `M6-002`.
...
5. Test in-process command/query ports, concurrent same-epoch same-host clones, in-place database rollback after grants/budget/revocation/effect changes, fixed OS-lock exclusion, sealed-generation/root mismatch, registry lease fencing, registry verifier rotation/revocation/compromise/freeze, prepared/finalized anchor crash points, exclusive takeover into a new epoch, externally anchored close-before-activation, restored pre-handoff source quarantine, authority handoff recovery, authorization-snapshot tampering, signing-key rotation/revocation/compromise-effective verification, revoked-key signing denial, cross-workspace/project/audience denial, receipt persistence failure, idempotency revocation/mismatch/concurrency, per-domain import repetition/rollback, and client-surface parity.

Checkpoint: equivalent authenticated requester, transport, acting-agent authority, deployment-instance key and exact external/sealed authority generation/root produce the same project create/read decision across every enabled client surface. A concurrent clone, in-place rollback, missing fixed OS lock, anchor mismatch, revoked/expired instance lease, rolled-back/expired/compromised registry key, restored database without exclusive new-epoch takeover, destroyed/revoked epoch key, disabled adapter, mismatched transport principal, tampered client or authorization snapshot, different workspace/agent/audience, revoked retry, obsolete signing key, or mismatched replay is denied; every decision has a historically verifiable authority-commit receipt and signed audit receipt/checkpoint.

### Milestone 2 — Outcome contracts and durable workflow graphs
1. Add immutable outcome-contract versions, pinned workflow graphs, nodes/dependencies, fenced leases, attempts, checkpoints, recovery decisions, typed lineage nodes/edges, immutable cryptographically committed effect-payload versions, canonical budget periods/accounts/parent-closure/reservations/settlements/permit bindings, versioned unique kill-switch rows, fully authorization/payload-bound external-effect intents/permits/outboxes/attempts, and one-per-intent effect-outbox tables through `M2-001`–`M2-004`.
...
6. Add the centralized effect gateway: resolvable immutable authorization snapshots plus current revocation checks, external/sealed authority-generation checks, canonical lock ordering, exact immutable payload decrypt/recanonicalize/HMAC verification, non-overlapping budget-period and full ancestor-reservation closure checks, semantic-key mismatch rejection, provider idempotency, `outcome_unknown` reconciliation/at-most-once handling, and explicit cancellation/compensation. Add per-domain importers for V1 outcomes, run history, artifacts, and conservative budget ceilings.

Checkpoint: restart preserves queues/events/checkpoints only when database and external/sealed authority generation/root match; dependencies and limits gate execution; stale authority/lease/generation fences cannot mutate state or duplicate an external effect; ciphertext, canonical-plaintext commitment or schema/canonicalization changes fail before dispatch; one-per-intent and attempt-number constraints reject duplicates; overlapping periods, illegal/cyclic parentage, incomplete ancestor reservations, kill-switch and budget races serialize/fail closed; unknown provider outcomes never auto-retry without idempotency; completion requires the pinned outcome version and a verified immutable authorization/payload/effect/lineage closure.
...
3. Implement project create/read, work submission, persisted event timeline, outcome criteria, evidence, lineage, approvals, limits, and kill-switch controls.
4. Add API-backed Playwright for pairing misuse, wrong Host/origin, CSRF, stale/rotated/revoked sessions, restart/reconnect and duplicate tabs/processes, plus Vitest, accessibility, responsive and hostile-preview-origin tests.

Checkpoint: the actual browser client pairs with and reconnects to the local daemon, survives authorized rotation/restart, rejects every hostile pairing/session/origin case, and completes the fake-provider project/work/evidence flow using only generated contracts and backend-reported state/capabilities. Only now is local-daemon web support enabled.
...
- Matrix: exact allow, no-grant deny, explicit deny wins, wrong principal/workspace/project/thread/audience, missing deployment-instance key/OS lock/lease, stale external/sealed generation or state root, restore quarantine, registry verifier rollback/expiry/revocation, expired/revoked requester or agent grant, authorization-snapshot tamper, delegation overreach, placement/credential mismatch, budget/runtime exhaustion, kill switch, and enabled-client-surface parity.
...
8. Deployment/workspace/client/execution/model/credential contracts cannot express an overloaded profile or plaintext secret; every authoritative commit advances an external registry or device-bound sealed generation/state root under one deployment-instance lease/OS lock, database rollback or same-epoch clones fail closed, registry verifier history is pinned/versioned, handoff revokes/closes the former instance before new-epoch activation, and restores default to read/queue-only quarantine.
...
12. Allow and deny decisions create immutable signed authorization snapshots and requester/agent/deployment-instance/authority-generation-attributed receipts with a unique workspace sequence and durable signing-key version; external/sealed generation advance, append, signature, or chain-head persistence failure prevents the protected action or quarantines ambiguous recovery.
...
15. Empty audit chains are invalid; checkpoints bind the authorizing snapshot/set and effect-payload-set digests plus deterministic workspace sequence, deployment instance, external/sealed authority generation/state root/commit receipt, epoch credential, signing-key version and compromise-effective history; artifact lookups accept only canonical SHA-256 digests.
...
- `local_daemon` CLI support is unavailable until Milestone 4 passes same-user bootstrap, deployment-instance activation, loopback/origin controls, credential rotation, lifecycle/recovery and retained-CLI reconnect tests; daemon web support remains unavailable until Milestone 5's actual generated-client/Playwright pairing, CSRF, rotation, restart and reconnect gate passes.
...
| Layer | Tests |
|---|---|
| Configuration/domain | allowed/invalid combinations, deployment-instance activation, fixed OS-lock/registry-lease singleton, external/sealed generation/root CAS, same-epoch clone and in-place rollback denial, registry verifier rotation/revocation/compromise/freeze, close-before-activate handoff, restore quarantine/new-epoch takeover, capability projection, canonical digests, discriminated scope parentage, audience snapshots, state transitions |
| Authorization/audit | requester/agent/delegation intersection, exact authority generation/root/commit receipt, signed immutable decision snapshots, current revocation, deny precedence, signing-key rotation/revocation/compromise history, limits, kill switches, enabled-client parity |
| Persistence/migration | V1 golden/quarantine capture, version-aware bootstrap, per-domain repeated imports, anchor prepare/finalize crash recovery, concurrent clone, in-place rollback, restored pre-handoff source, rollback fixtures, uniqueness/check constraints, transactions, sequenced signed receipt/event chains, tamper and isolation |
| Outcomes/workflows/effects | pinned criteria/evidence, immutable authorization/payload/effect/lineage closure, ciphertext/plaintext-commitment tamper, claim races, fenced leases, canonical semantic idempotency, non-overlapping budget periods, legal acyclic single-parent closure, complete normalized ancestor reservations, one-per-intent permit/outbox, unique attempts, materialized kill locks, `outcome_unknown`, reconciliation, transactional outbox, compensation, stuck recovery, dependencies, cancellation |
...
| API/local daemon/events | OIDC/session/CSRF/token protocol tests, local same-user deployment-instance bootstrap, retained-CLI rotation/origin/Host/lifecycle/recovery/reconnect, protocol-level browser pairing endpoints, 403/404 behavior, idempotency, OpenAPI, scoped cursor replay; no web-support claim |
...
| Web/channels | actual generated-client API-backed Playwright for one-time daemon pairing, SameSite/CSRF, rotation/revocation, restart/reconnect and hostile origins; accessibility, hostile preview, signed channel ingress, dedupe, step-up approval |
...
- **Database rollback or clone resurrects authority:** every authority-bearing commit advances a registry-backed or device-sealed generation/state root outside the workspace database. One deployment-instance lease or fixed workspace OS lock is exclusive; registry verifier history is pinned/versioned; stale roots, concurrent clones, copied key locators and in-place rollback quarantine. Restore mutation requires former-instance revocation and exclusive new-epoch takeover.
- **Stale worker, payload substitution, budget write-skew or duplicate effects:** intent/permit/outbox/attempt/audit/lineage bind one immutable content-addressed payload version, ciphertext digest, encryption/commitment key versions and canonical-plaintext HMAC verified immediately before dispatch. Canonical locks, non-overlapping periods, legal acyclic account closure, normalized full-ancestor permit reservations, unique kill/permit/outbox/attempt constraints and exact authority generation close local races. Unknown provider outcomes never auto-retry without provider idempotency; compensation is remediation only.
...
- **Local-daemon exposure or premature web-support claim:** Milestone 4 can enable only retained-CLI daemon use after instance/bootstrap, loopback/Host/origin, rotation/revocation, lifecycle and crash-recovery proof. Browser support waits for Milestone 5's actual generated web client and API-backed Playwright pairing/CSRF/rotation/restart/reconnect gate.
...
- **Review availability:** Claude returned HTTP 401, Gemini required Antigravity migration, and Codex timed out without a verdict. Earlier paired Hermes rounds and their responses are recorded in `PLAN-REVIEW-LOG.md`. Exact commit `134475ea8a1c5fe0c94f815f2575647240499d35` received two `VERDICT: REVISE` results with four high findings: authority singleton/anti-rollback, immutable effect payload commitment, enforceable budget hierarchy, and real-web daemon sequencing. All are incorporated into this new candidate. Both fresh scopes must return `VERDICT: PASS` on the same exact commit; no timeout, partial result or single pass is approval.
