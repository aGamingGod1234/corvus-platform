...
| `corvus review`, approval/apply, `corvus undo` | `DeliveryQueryPort`, `ApprovalCommandPort`, `ApplyCommandPort`, `UndoCommandPort` | Milestone 3 routes review/apply/undo through request context, current authorization/revocation, effect fencing, audit and rollback services | Milestone 4 routes retained commands over HTTP; Milestone 5 adds generated-web controls; Milestone 10 reruns both clients and Milestone 11 reruns Tauri | approval mismatch/replay/expiry, apply conflict, crash recovery, current-revocation denial, one-time effect, undo/compensation and receipt parity |
...
- Review, approval, filesystem apply and undo are authority-bearing effects. They use delivery/approval/undo ports, current `RequestContext` authorization and revocation, the centralized effect gateway, immutable receipts, conflict/crash handling and auditable compensation; no adapter may call `DeliveryManager` directly.
...
6. Cut retained `corvus review`/approve/apply and `corvus undo` to delivery/approval/apply/undo ports. Test current-revocation denial, manifest mismatch/replay/expiry, conflict, one-time fenced apply, injected crash recovery, undo/compensation and audit/rollback receipt parity in isolated filesystem fixtures.
...
- `embedded_local` CLI support is unavailable until Milestone 3 cuts actual retained chat/run/TUI and review/apply/undo to the in-process client and proves compatibility, cancellation/event parity, current authorization, conflict/crash recovery and undo receipts.
...
| Delivery | delivery/approval/apply/undo port contracts, current authorization/revocation, fenced one-time effect, manifest/artifact rehash, approval binding, locking, conflict/crash atomicity, backup/undo/compensation and receipt parity through real clients, malicious paths |
...
- **Bundle TOCTOU/crash windows:** rehash at apply, durable one-time approvals, locks, crash-point testing and delivery/approval/apply/undo port cutover are mandatory; no retained command or UI calls the manager directly.
...
- **Review availability:** Claude returned HTTP 401, Gemini required Antigravity migration, and Codex timed out without a verdict. Earlier paired Hermes rounds and their responses are recorded in `PLAN-REVIEW-LOG.md`. For exact commit `0f4a560863e06d7c886fd8db1b16c0c6ae621984`, the security scope timed out with no verdict and the product scope returned `VERDICT: REVISE` with three high findings: retained conversation/TUI cutover, authoritative delivery/apply/undo cutover, and self-host packaging before support. All product findings are incorporated; the timed-out security scope must be rerun. Both fresh scopes must return `VERDICT: PASS` on the same exact commit; no timeout, partial result or single pass is approval.
